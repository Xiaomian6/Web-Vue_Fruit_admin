<template>
  <div class="app-container">
    ...
    <el-table
      ...
      style="width: 100%;"
    >
      <el-table-column :label="'收购方ID'" prop="id" align="center" width="80px">
        <template slot-scope="scope">
          <span>{{ scope.row.user_id }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="'收购方名'" align="center" min-width="100px">
        <template slot-scope="scope">
          <span>{{ scope.row.username }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="'收购方地址'" min-width="160px" align="center">
        <template slot-scope="scope">
...
<script>
  import { buyList } from '@/api/acquisitionadmin'
  ...
  export default {
    ...
    data() {
      return {
        ...
        listQuery: { // 创建一个列表
          ...
          limit: 20,
          user_id: 0, // 收购方ID
          username: undefined, // 收购方名
          user_address: undefined, // 收购方地址
    ...
    methods: {
      getList() { // 抓取数据api
        ...
        buyList(this.listQuery).then(response => {
